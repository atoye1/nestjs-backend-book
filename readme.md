...
다른 프로바이더를 상속받은 프로바이더를 주입할 때는 @Inject() 데커레이러를 활용한 속성기반프로바이더를 사용하면 간편하다.
...
가드는 CanActivate 인터페이스를 구현하여 만들 수 있다. 가드 클래스의 메서드인 canactivate()안에서 `context.switchToHttp().getRequest();`와 같은 형식으로 execution context에 접근할 수 있으므로 req객체를 불러와서 입맛대로 가공할 수 있다.
...
내장로거와 LoggerService 인터페이스를 구현한 커스텀 로거 모두 활용가능하다.  
하지만 상용 수준으로 서비스 운영을 위해선 로그를 외부서비스로 전송하는 기능이 필요하다.  
이를 위해서 nest-winston을 사용하는 것이 편리하다.

`npm i nest-winston winston`

윈스턴 로거를 사용하는 방법은 세 단계가 있다.

1. app 모듈에다 임포트해서 컨트롤러나 서비스에 주입해 사용(특정 서비스 로거로만 활용)
2. main.ts에 전역 로거로 설정한 뒤 목적지에서 활용하는 방법
3. 2번에 더해서 부트스트래핑까지 윈스톤로거로 사용하는 방법

로깅 포맷의 일관성을 위해서 1+ 2 + 3 번을 모두 적용하는 것이 바람직하다.
다음과 같이 진행하면 된다.

0. app.module.ts에 동적모듈로 winstonLogger를 설정한다.
1. main.ts에서 윈스톤을 전역 로거로 설정한다. 설정을 별도의 파일로 분리하면 더욱 깔끔하다.
2. 로거 서비스를 사용하고자 하는 모듈에서 `providers: [Logger]` 선언해준다.
3. 사용하고자하는 컨트롤러나 프로바이더에서 LoggerService를 `@Inject(WINSTON_MODULE_NEST_PROVIDER)`로 주입해서 사용한다.

### 12. 모든 것은 항상 실패한다 : 예외 필터

네스트는 기본적으로 전역예외필터가 있지만 기능이 풍부하지 않아 보통 커스텀 예외필터를 하나 추가해서 전역에 등록한 뒤 사용한다.
부트스트랩 과정에서 등록하는 예외필터는 의존성을 주입할 수 없다. 의존성 주입이 불가능하면 앞에서 작성한 커스텀 로거도 사용할 수 없다.
이를 보완하기 위해 `app.module.ts`에 예외필터를 커스텀 프로바이더로 선언한다. 그러면 의존성이 주입된 예외필터의 인스턴스를 네스트가 자동으로 생성해준다.

의존성을 주입할때 클래스나 인터페이스는 @Inject() 데커레이터를 사용하지 않는다.  
config파일과 같이 클래스나 인터페이스가 아닌 경우는 @Inject()를 사용한다. 속성기반으로 프로바이더 주입시 @Inject활요한다. 상속관계에 있는 클래스의
